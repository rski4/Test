...
KZonePitcherBPen = function(df = bp, player = "Leroy Jenkins (#99)", speed = FALSE, pitch.type = FALSE){
  ...
  if(speed & pitch.type)
    ...
    xlim(-3, 3) + xlab("") +
    ylim(0, 6) + ylab("") +
    geom_point(data = df[which(df$pitcher == player),], aes(x = px, y = pz, shape = pitch.type, color = pitch.speed)) +
    ...
    xlim(-3, 3) + xlab("") +
    ylim(0, 6) + ylab("") +
    geom_point(data = df[which(df$pitcher == player),], aes(x = px, y = pz, color = pitch.speed)) +
    geom_rect(data = k.zone,
              aes(xmin = x1, xmax = x2, ymin = y2, ymax = y1), 
              color = "grey20", fill = "white", alpha = .001) +
    scale_color_continuous(low = "red", high = "red4") +
    ...
    xlim(-3, 3) + xlab("") +
    ylim(0, 6) + ylab("") +
    geom_point(data = df[which(df$pitcher == player),], aes(x = px, y = pz, shape = pitch.type)) +
    geom_rect(data = k.zone,
              aes(xmin = x1, xmax = x2, ymin = y2, ymax = y1), 
              color = "grey20", fill = "white", alpha = .001) +
    ...
    xlim(-3, 3) + xlab("") +
    ylim(0, 6) + ylab("") +
    geom_point(data = df[which(df$pitcher == player),], aes(x = px, y = pz)) +
    geom_rect(data = k.zone,
              aes(xmin = x1, xmax = x2, ymin = y2, ymax = y1), 
              color = "grey20", fill = "white", alpha = .001) +
    scale_color_continuous(low = "red", high = "red4") +
